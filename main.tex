...
Trong những năm gần đây, robot di động (Mobile Robot) ngày càng được nghiên cứu và ứng dụng rộng rãi trong ngành tự động hóa. Từ các robot vận chuyển hàng hóa trong nhà máy, robot phục vụ trong nhà hàng, đến các robot thám hiểm địa hình phức tạp -- tất cả đều đòi hỏi khả năng di chuyển ổn định và thích ứng với các điều kiện tải trọng thay đổi.
...
\textbf{Về tải trọng:} Cảm biến loadcell có tải trọng định mức 50kg mỗi cụm. Con trượt được thiết kế cho phép đặt đối trọng với khối lượng tùy chọn -- khả năng cân bằng tỷ lệ thuận với khối lượng đối trọng sử dụng.
...
Phương pháp khối lượng di động được chọn dựa trên đặc điểm ứng dụng và các ràng buộc thực tế. Khung đế robot cần cân bằng khi tải thay đổi (như cánh tay robot gắp vật), không đòi hỏi thời gian đáp ứng cực nhanh như robot tự cân bằng động. Về mặt năng lượng, sau khi đạt vị trí cân bằng, động cơ bước dừng hoàn toàn và giữ vị trí bằng momen từ tính, tiết kiệm hơn đáng kể so với phương pháp bánh đà phải quay liên tục.

Việc sử dụng loadcell cho phép đo trực tiếp chênh lệch trọng lượng giữa hai bên, cung cấp phản hồi chính xác mà không cần tính toán phức tạp từ góc nghiêng. Các linh kiện cần thiết (động cơ bước, vitme, loadcell, Arduino) đều phổ biến và có giá thành hợp lý tại Việt Nam. So với phương pháp bánh đà hoặc CMG, phương pháp này có độ phức tạp cơ khí và điều khiển thấp hơn, phù hợp với khuôn khổ đồ án tốt nghiệp.
...
Phương trình này cho thấy một đặc điểm quan trọng: khả năng cân bằng phụ thuộc vào khối lượng đối trọng $m_{slider}$ đặt trên con trượt. Với hành trình giới hạn $d_{slider,max}$, độ lệch tải tối đa có thể bù đắp là:
...
Với cấu hình khung hiện tại: $d_{slider,max} = 120$mm và $d_{load} = 175$mm (một nửa chiều rộng khung 35cm), hệ số chuyển đổi là:

\begin{equation}
\frac{m_{load,max}}{m_{slider}} = \frac{120}{175} \approx 0.69
\end{equation}

Điều này có nghĩa: độ lệch tối đa có thể bù $\approx 69\%$ khối lượng đối trọng. Ví dụ, với đối trọng 500g, có thể bù độ lệch tối đa khoảng 343g.

Kết quả này cho thấy để tăng khả năng cân bằng, có thể: (1) tăng khối lượng đối trọng, (2) tăng hành trình di chuyển, hoặc (3) bố trí tải gần tâm hơn. Thiết kế con trượt cho phép thay đổi đối trọng linh hoạt tùy theo yêu cầu ứng dụng.
...
Dự án sử dụng thư viện \texttt{HX711\_ADC} của tác giả Olav Kallhovd [2]  thay vì thư viện \texttt{HX711} chuẩn (của Bogde). Khác biệt chính nằm ở cơ chế đọc dữ liệu:
...
Vi bước là kỹ thuật điều khiển dòng điện trong các cuộn dây theo dạng sóng sin/cosin thay vì dạng bước vuông. Kỹ thuật này cho phép chia nhỏ mỗi bước đầy đủ thành nhiều bước nhỏ hơn.
...
Do đó, việc in Serial không kiểm soát có thể làm động cơ bước ``chậm như bị mắc kẹt'' -- hiện tượng được ghi nhận trong quá trình phát triển hệ thống.
...
\begin{itemize}
    \item Nếu PID tính ra tốc độ $> 0$ nhưng $< 2000$ bước/s: Ép tốc độ lên 4000 bước/s
    \item Nếu PID tính ra tốc độ $= 0$ hoặc $\geq 4000$: Giữ nguyên
\end{itemize}

Ngưỡng này đảm bảo khi động cơ cần chạy, nó chạy ở tốc độ đủ cao để vượt qua vùng cộng hưởng.
...
\textbf{Vấn đề 2 - Bật/tắt liên tục (Chattering):} Nếu chỉ dùng một ngưỡng 100g, khi sai số dao động quanh ngưỡng này, hệ thống sẽ bật/tắt liên tục. Giải pháp là áp dụng Hysteresis $\pm$20g với ngưỡng bật 110g và ngưỡng tắt 90g.
...
\begin{enumerate}
    ...
    \item \textbf{Tìm chậm}: Động cơ chạy chậm (6000 bước/s) về phía công tắc để xác định vị trí chính xác.
...
\textbf{Nhận xét}: Hệ thống đáp ứng tốt với tải tĩnh trong phạm vi 100--500g. Thời gian đáp ứng tỷ lệ thuận với độ lớn của tải. Độ lệch cuối cùng đều nằm trong vùng chấp nhận (±100g).
...
Khả năng cân bằng phụ thuộc vào khối lượng đối trọng đặt trên con trượt. Với khung có chiều rộng 350mm (loadcell đặt ở mép, cánh tay đòn tải $d_{load} = 175mm$) và hành trình tối đa $d_{slider,max} = 120mm$, công thức momen cân bằng:

$$m_{slider} \times d_{slider} = m_{load} \times d_{load}$$

Suy ra độ lệch tối đa có thể bù với đối trọng $m_{slider}$:

$$m_{load,max} = m_{slider} \times \frac{d_{slider,max}}{d_{load}} = m_{slider} \times \frac{120}{175} \approx 0.69 \times m_{slider}$$

Bảng \ref{tab:balance_capacity} trình bày khả năng cân bằng theo khối lượng đối trọng:
...
\begin{tabular}{|c|c|c|}
\hline
\textbf{Đối trọng (g)} & \textbf{Độ lệch tối đa bù được (g)} & \textbf{Ghi chú} \\
\hline
200 & $\sim$137 & Dùng trong thử nghiệm \\
300 & $\sim$206 & -- \\
500 & $\sim$343 & Khuyến nghị cho ứng dụng nhẹ \\
1000 & $\sim$686 & Khuyến nghị cho ứng dụng nặng \\
1500 & $\sim$1029 & Sử dụng pin/acquy làm đối trọng \\
\hline
\end{tabular}
\end{table}

\textbf{Nhận xét}: Thiết kế con trượt cho phép thay đổi khối lượng đối trọng linh hoạt tùy theo ứng dụng cụ thể. Trong thực tế, có thể tận dụng pin, acquy hoặc các thành phần nặng của robot làm đối trọng để vừa tối ưu không gian vừa tăng khả năng cân bằng.

\section{Phân tích kết quả}

\subsection{Hiệu năng bộ điều khiển PID}

Với bộ tham số PID đã tinh chỉnh ($K_p = 40$, $K_i = 0.05$, $K_d = 0$):
...
Cơ chế Deadzone (±100g) và Hysteresis (±10g) mang lại các lợi ích:
...
\begin{itemize}
    ...
    \item \textbf{Tránh hunting}: Ngưỡng bật ($>$110g) khác ngưỡng tắt ($<$90g) ngăn dao động
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Độ chính xác}: Độ lệch cuối cùng $<$100g, phù hợp ứng dụng thực tế
...
\begin{enumerate}
    ...
    \item \textbf{Phụ thuộc khối lượng đối trọng}:
    \begin{itemize}
        \item Khả năng cân bằng = $0.69 \times$ khối lượng đối trọng (với cấu hình khung hiện tại)
        \item Giải pháp: Chọn đối trọng phù hợp với tải dự kiến; tận dụng pin/acquy của robot
    \end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Thuật toán PID với các tham số đã tinh chỉnh ($K_p=40$, $K_i=0.05$, $K_d=0$)
        \item Cơ chế Deadzone (±100g) và Hysteresis (±10g)
    ...
    \begin{itemize}
        ...
        \item Độ chính xác ±100g (vượt yêu cầu ±100g)
...
\begin{enumerate}
    ...
    \item \textbf{Khả năng cân bằng}: Phụ thuộc vào khối lượng đối trọng đặt trên con trượt. Với hệ số chuyển đổi $\approx 0.69$, cần đối trọng lớn hơn độ lệch muốn bù (ví dụ: đối trọng 500g bù được $\sim$343g độ lệch). Cần chọn đối trọng phù hợp với tải dự kiến của từng ứng dụng.
...
\begin{enumerate}
    ...
    \item \textbf{Tăng khả năng cân bằng}: Sử dụng đối trọng nặng hơn (1--1.5kg) hoặc tận dụng pin/acquy của robot làm đối trọng di động. Theo tính toán, đối trọng 1.5kg có thể bù được độ lệch đến $\sim$1kg.
 ...
 * 3. Deadzone: Vung chet +/- 100g giup dong co nghi ngoi khi da can bang.
...
// --- CAU HINH CHONG RUNG ---
const float MIN_SPEED_THRESHOLD = 4000.0;
...
const float ACCEPTABLE_RANGE = 100.0;
